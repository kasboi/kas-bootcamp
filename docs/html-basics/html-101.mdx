...
It's great to have one central location for all the projects we will be handling in this bootcamp. You can follow my steps to ensure consistency
...
3. Open VS Code
4. Open the `html_basics` folder in the code editor and create a new HTML file named `index.html`
5. Type ! to generate boiler html code (This is an emmet shortcut, more o come), this is power of Emmet, a built in vs code tool for fast code generation using shortcuts. briefly explain what Emmet is and show them the web page. notice that it is still blank but the title of the tab has changed.

```html title="/html_basics/index.html"
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8" />
...
`html`, `meta`, `title`, `body` are examples of HTML tags. They are used to describe the kind of behaviour we want to exhibit to the browser for example headings, ordered or undordered lists.

## HTML Elements

HTML elements are everything from the start tag to the end tag
...
:::tip
Use the auto-save feature `onFocusChange` to save your files automatically when you switch from code editor to browser. This + live server is a powerful combo

`ctrl + ,` to go to settings → File (auto save) → onFocusChange
:::
